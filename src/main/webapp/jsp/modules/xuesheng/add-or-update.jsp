...
<body>
						...
						<div class="widget-area-2 lochana-box-shadow">
							...
							<form id="addOrUpdateForm">
								<div class="form-row">
									...
									<div class="form-group col-md-6">
										<label>宿舍号</label>
										<select id="sushehaoSelect" name="sushehao" class="form-control">
										</select>	
									</div>	
									<div class="form-group col-md-6">
										<label>联系方式</label>
	...
	<script>
		...
		function banjimingchengSelect(){
			http("option/banji/banjimingcheng","GET",{},(res)=>{
				if(res.code == 0){
		...
		function sushehaoSelect(){
			http("option/sushexinxi/sushehao","GET",{},(res)=>{
				if(res.code == 0){
					sushehaoOptions = res.data;
					var nullOption = document.createElement('option');
					var sushehaoSelect = document.getElementById('sushehaoSelect');
					sushehaoSelect.appendChild(nullOption);
					for(var i=0;i<sushehaoOptions.length;i++){
						var sushehaoOption = document.createElement('option');
						sushehaoOption.setAttribute('name','sushehaoOption');
						sushehaoOption.setAttribute('value',sushehaoOptions[i]);
						sushehaoOption.innerHTML = sushehaoOptions[i];									
						if(ruleForm.sushehao == sushehaoOptions[i]){
							sushehaoOption.setAttribute('selected','selected');
						}
						sushehaoSelect.appendChild(sushehaoOption);
					}
				}
			});
		}	
		...
		function submit() {
                //获取表单数据前取消禁用
                $("#banjimingchengSelect").attr('disabled',false);
                //获取表单数据前取消禁用
                $("#sushehaoSelect").attr('disabled',false);
		...
		function dataBind(){
			...
			var sushehaoOptions = document.getElementsByClassName("sushehaoOption");
			for(var sushehaoCount = 0; sushehaoCount < sushehaoOptions.length;sushehaoCount++){
				if(sushehaoOptions[sushehaoCount].getAttribute('value') == ruleForm.sushehao){
					sushehaoOptions[sushehaoCount].setAttribute('selected','selected');
				}
			}
			...
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.sushehao != null && res.data.sushehao != ''){
						...
						var sushehaoOptions = document.getElementsByClassName("sushehaoOption");
						for(var sushehaoCount = 0; sushehaoCount < sushehaoOptions.length;sushehaoCount++){
							if(sushehaoOptions[sushehaoCount].getAttribute('value') == res.data.sushehao){
								sushehaoOptions[sushehaoCount].setAttribute('selected','selected');
							}
						}
					}
		...
		function readonly(){
            if(window.sessionStorage.getItem('role')!="管理员") {
                ...
                $("#banjimingchengSelect").attr('disabled',true);
                $("#sushehaoSelect").attr('disabled',true);
            }
		}
